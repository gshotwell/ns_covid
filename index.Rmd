...
get_immune <-  function(check_date, df = vaccines) {
  df %>% 
    ...
    mutate(days = row_number(), 
           immune_pct = case_when(
             days >= 49 ~ 0.64,
             days >= 42 & days < 48 ~ 0.70,
             days >= 35 & days < 42 ~ 0.71,
             days >= 28 & days < 35 ~ 0.68,
             days >= 21 & days < 28 ~ 0.60,
             days >= 14 & days < 21 ~ 0.48,
             TRUE ~ 0.03
           )
    ) %>% 
    mutate(immune_pop = change_vaccinations * immune_pct) %>% 
    pull(immune_pop) %>% 
    sum()
}
...
ggplot(vaccines, aes(x = date, y = immune)) + 
  ...
  labs (title = "Nova Scotia Aggregate immunity",
        subtitle = "Based on an estimated effectiveness of one vaccine dose",
