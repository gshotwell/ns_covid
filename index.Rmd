...
resp <- GET("https://api.covid19tracker.ca/reports/province/pe")
covid_pe <- tibble(raw = content(resp)$data) %>% 
  unnest_auto(raw)
covid <- covid %>% 
...
vaccines %>% 
  ...
  select(date, `Reserved for second dose` = reserved, `Just sitting in the freezer` = unused) %>% 
...
plot_df %>% 
  ...
  labs(title = "Many more people are vaccinated than have Covid",
       y = "Full vaccinations per case",
       x = "Date") +
  theme(panel.grid.minor = element_blank())
