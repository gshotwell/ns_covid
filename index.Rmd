...
covid %>% 
  ...
  labs(y = "Active Cases",
       ...
       title = "Nova Scotia's third wave is similar to the first",
...
ggplot(covid, aes(x = date_num, y = roll_cases, group = wave, colour = wave)) +
  ...
  labs(y = "New cases (7 day average)",
       x = "Days since outbreak began",
       title = "Nova Scotia's thurd waves is similar to the first") +
...
# covid %>% 
#  select(wave, date_num, change_tests, active) %>% 
#   mutate(roll_tests = roll_sum(change_tests, 7, fill = NA, align = "right")) %>% 
#   mutate(test_per_active = roll_tests / active) %>% 
#   filter(active > 10) %>% 
#   filter(date_num < 50) %>% 
#   ggplot(aes(x = date_num, y = test_per_active, 
#              group = wave, colour  = wave)) +
#   geom_line()

covid %>%
 select(wave, date_num, change_tests, active, roll_cases) %>%
  mutate(roll_tests = roll_mean(change_tests, 7, fill = NA, align = "right")) %>%
  mutate(positivity = roll_cases / roll_tests) %>%
  ggplot(aes(x = date_num, y = positivity,
             group = wave, colour  = wave)) +
  geom_line() +
