...
raw_data$Date[293 : nrow(raw_data)] <- seq.Date(ymd("2021-01-01"), ymd("2021-01-31"), by = "day")[nrow(raw_data) - 293]
...
case_df <- covid %>% 
  ...
  mutate(eight_day = roll_sum(new_cases, 8, fill = NA, align = "right"))
...
plot_df <- covid %>% 
  ...
  mutate(positive_rate = new_cases / daily_tests,
         positive_rate_roll = roll_mean(positive_rate, 7, fill = NA, align = "right", na.rm = TRUE)) 
...
vaccines <- resp %>% 
  unnest_auto(raw) %>% 
  select(date, change_vaccinations, change_vaccines_distributed, total_vaccinations)
...
plot_df %>% 
  select(date, vaccinated, vaccine_distributed) %>% 
  rename(`Doses administered` = vaccinated,
         `Doses received` = vaccine_distributed) %>% 
  pivot_longer(-date) %>% 
  ggplot(aes(x = date, y = value, group = name, colour = name)) +
  geom_path() +
  labs(y = "Sevem day average", 
       x = "Date",
       title = "Nova Scotia Vaccination Rate")

plot_df %>% 
  ggplot(aes(x = date, y = vaccine_ratio)) +
  geom_line() +
  labs(title = "Vaccinations per active case",
