...
covid <- covid %>% 
  ...
  mutate(wave = case_when(
    date < "2020-10-21" ~ "Wave 1",
    date > "2021-02-15" ~ "Wave 3", 
    TRUE ~ "Wave 2")
  ) %>% 
  arrange(date) %>% 
  group_by(wave) %>% 
...
vaccines %>% 
  ...
  select(date, `All stored doses` = in_freezer, `Sitting in the freezer` = unused, `Reserved for second dose` = reserved) %>% 
  pivot_longer(-date, names_to = "Type", values_to = "value") %>% 
  ggplot(aes(x = date, y = value, group = Type, colour = Type)) +
  geom_path() +
  scale_y_continuous(breaks = c(vaccines$unused[nrow(vaccines)], vaccines$reserved[nrow(vaccines)], vaccines$in_freezer[nrow(vaccines)]),
                     labels = scales::comma) +
